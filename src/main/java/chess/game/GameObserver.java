...
public abstract class GameObserver {
    public static boolean InCheck(PieceColor color, Board subject) {
        for (int file = 0; file < 8; file++) {
            for (int rank = 0; rank < 8; rank++) {
                Piece currPiece = subject.getPiece(file, rank);
                if (currPiece.getColor() != color) {
                    List<Move> currPieceMoves = currPiece.getMoves(subject, file, rank);
    ...
    public static boolean InCheckmate(PieceColor color, Board subject) {
        return LegalMoves.GetLegalMoves(subject, color).isEmpty();
    }
}
